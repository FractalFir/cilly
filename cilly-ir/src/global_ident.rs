use std::fmt::Write;
use nom::{Parser, branch::alt, bytes::{complete::take, tag}, character::complete::satisfy, combinator::{map, map_res}, multi::many0, sequence::preceded};
...
impl qparse::Parseable<qparse::Display> for GlobalIdent {
    fn parse(input: &str) -> nom::IResult<&str, Self> {
        (
            ...
            std::iter::once(prefix).chain(body).flatten().collect::<Vec<_>>()
        })
        .map_opt(|v|
            String::from_utf8(v).ok().map(|name|Self{name})
        )
        .parse(input)
...
fn escaped_byte(input: &str) -> nom::IResult<&str, u8> {
    map_res(preceded(tag("\\"), take(2usize)), 
...
fn ident_char<'a>(is_first: bool) -> impl FnMut(&'a str) -> nom::IResult<&'a str, Vec<u8>> {
    move |input| {
        alt((
            ...
            // or a raw one.
            map(satisfy(move |c| IDENT_CHARS.contains(c) || (c.is_ascii_digit() && !is_first)), |c| {
                let mut buf = [0u8; 4];
